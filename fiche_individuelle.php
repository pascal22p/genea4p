...
if ($_SESSION['permission']->permission[_PERM_EDIT_FILES_])
{
    ...
    echo ' <a href="',g4p_make_url('admin','exec.php','g4p_opt=del_cache&g4p_id='.$g4p_indi->indi_id,0),'" class="admin">',$g4p_langue['menu_recreer_cache'],'</a> 
        <a href="',g4p_make_url('','g4p_object.php','id='.$g4p_indi->indi_id,0),'" target="_blank" class="admin">G4p_object</a> ';
}
...
if($g4p_indi->timestamp!='0000-00-00 00:00:00' and !empty($g4p_indi->timestamp))
...
//resn
    ...
    if($_SESSION['permission']->permission[_PERM_EDIT_FILES_])
    {
        if($g4p_indi->resn!='privacy')
            $link=' <a href="'.g4p_make_url('admin','exec.php','g4p_opt=mask_fich&amp;g4p_id='.$g4p_indi->indi_id,0).'" class="admin">'.$g4p_langue['menu_masque_fiche'].'</a>';
        else
            $link=' <a href="'.g4p_make_url('admin','exec.php','g4p_opt=demask_fich&amp;g4p_id='.$g4p_indi->indi_id,0).'" class="admin">'.$g4p_langue['menu_demasque_fiche'].'</a>';
    }
if($g4p_indi->resn)
...
if(!empty($g4p_indi->events))
{
    ...
    foreach($g4p_indi->events as $g4p_a_ievents)
    {
        ...
        if($_SESSION['permission']->permission[_PERM_EDIT_FILES_])
            echo '<a href="'.g4p_make_url('','modification_event.php','id_event='.$g4p_a_ievents->id,0).'" class="admin">',$g4p_langue['menu_mod_event'],'</a>';
        echo '</dt>';
