...
if(!empty($_GET['q']))
{
    ...
    $g4p_query=$g4p_mysqli->g4p_query("SELECT nccenr, ign_rgc.dep AS dep, ign_rgc.com, 
        CONCAT(ign_rgc.dep,ign_rgc.com) as insee,  longi_dms , lati_dms 
        FROM insee_communes
        LEFT JOIN ign_rgc ON insee_communes.dep=ign_rgc.dep AND insee_communes.com=ign_rgc.com
        WHERE actual=1 AND (ncc LIKE '".$_GET['q']."' OR
        nccenr LIKE '".$_GET['q']."')
        ORDER BY ncc
        LIMIT 40   ");
    ...
    #        $degre=$g4p_a_result['longi_dms'][$cpt-5].$g4p_a_result['longi_dms'][$cpt-6];
            /* $json[]='{ dep: "'.$g4p_a_result['dep'].'", '
                ...
                .' ville: "'.$g4p_a_result['nccenr'].'", '
                .'insee: "'.$g4p_a_result['insee'].'",  '
                .'longi_dms: "'.$g4p_a_result['longi_dms'].'",  '
                .'lati_dms: "'.$g4p_a_result['lati_dms'].'"} ';*/
                echo $g4p_a_result['nccenr'].'|'.$g4p_a_result['insee'].'|'.$dept[$g4p_a_result['dep']]['dept'].
                    '|'.$regions[$dept[$g4p_a_result['dep']]['region']].'|France|'.$g4p_a_result['longi_dms'].'|'.$g4p_a_result['lati_dms']."\n";
        }
        //echo '[ '.implode($json,',').' ]';
    }
}
