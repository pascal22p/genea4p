...
switch($_GET['type'])
{
  ...
  case 'indi_1':
    $sql="SELECT i.indi_id, i.indi_nom, i.indi_prenom, i.indi_sexe AS indi_sexe
    FROM `genea_individuals` AS i";
    if(isset($_GET['evt_lieu'])) 
    {
      $sql.=" JOIN rel_indi_event AS a ON ( i.indi_id=a.indi_id )";
      $sql.=" JOIN genea_events AS n ON ( a.events_id = n.id AND n.lieu<>'')";
    }
    $sql.=" WHERE i.base =".$_SESSION['genea_db_id']." AND indi_nom LIKE '%".mysql_escape_string($_POST['g4p_nom'])."%' AND indi_prenom LIKE '%".mysql_escape_string($_POST['g4p_prenom'])."%'";
    if($_SESSION['permission']->permission[_PERM_MASK_INDI_])
      $sql.=" AND (i.resn IS NULL OR i.resn<>'privacy')";

    if(isset($_GET['evt_lieu'])) 
      if($_GET['evt_lieu']!="_nonvide_") // j'aurai mis un if(!empty($_GET[])), ca eviterai d'avoir à ce trimbaler evt_lieu dans l'url tout le temps ou de l'initialiser
        $sql.=" AND n.lieu='".mysql_escape_string($_GET['evt_lieu'])."'";
    ...
    if($g4p_result=g4p_db_result_query($sql))
    {
      ...
      foreach($g4p_result as $a_g4p_result)
      {
        ...
        if(!$_SESSION['permission']->permission[_PERM_MASK_INDI_] or $a_g4p_result['indi_id']['resn']!='privacy')
        {
          ...
          echo $a_g4p_result['indi_id'].' <img src="'.$g4p_chemin.'images/'.$a_g4p_result['indi_sexe'].'.png" alt="Homme" class="icone_sexe" /> <a class="liste" href="',g4p_make_url('','index.php','g4p_action=fiche_indi&id_pers='.$a_g4p_result['indi_id'],'fiche-'.g4p_prepare_varurl(str_replace('?','_aa_',$a_g4p_result['indi_nom'])).'-'.g4p_prepare_varurl($a_g4p_result['indi_prenom']).'-'.$a_g4p_result['indi_id']),'">',$a_g4p_result['indi_nom'],' ',$a_g4p_result['indi_prenom'],'</a>',$g4p_texte;
          ...
          if(!empty($_POST['g4p_referer']) and !empty($_POST['g4p_champ']))
          {
            echo ' <b><a href="'.g4p_make_url($_POST['g4p_referer'][0],$_POST['g4p_referer'][1],$_POST['g4p_referer'][2].'&amp;'.$_POST['g4p_champ'].'='.$a_g4p_result['indi_id'],0).'">',$g4p_langue['recherche_indi_select'],'</a></b>';
          }
